defmodule StoneChallenge do
  ...
  def main(items_file_name \\ "shopping_list.json", emails_file_name \\ "email_list.json") do
    with {:ok, %{"items" => list_of_items}} <- get_content(@files_path <> items_file_name),
         {:ok, %{"emails" => list_of_emails}} <- get_content(@files_path <> emails_file_name),
  ...
  defp validate_min_length(list, min), do: if length(list) >= min, do: :ok, else: {:error, ""}

  @spec sum_items_value([%{String.t() => number()}]) :: number()
  defp sum_items_value(list_of_items) do
    list_of_items
  ...
  @spec division(number(), number()) :: {number(), number()}
